    ...
    public class CheckStockLevelsViewPresenter
    {
        ...
        void worker_DoWork(object sender, DoWorkEventArgs e)
        {
            ...
            using (var tricorn = new TricornDataProvider()) {
                var materials = tricorn.GetMaterials(searchValue).OrderBy(m => m.Name);

                foreach (var material in materials) {
                    var batchesGreaterThanZero = tricorn.GetMStocks(material.Material_Reference)
                        .Where(b => b.Quantity_In_Stock > 0);

                    double? totalQuantity = 0;

                    var parent = new CheckStockLevelsViewModel(material.Name, null, null, new List<CheckStockLevelsViewModel>());
                    
                    if (!batchesGreaterThanZero.Any()) {
                        continue;
                    }

                    foreach (var batch in batchesGreaterThanZero) {
