
//------------------------------------------------------------------------------
// <auto-generated>
//    This code was generated from a template.
//
//    Manual changes to this file may cause unexpected behavior in your application.
//    Manual changes to this file will be overwritten if the code is regenerated.
// </auto-generated>
//------------------------------------------------------------------------------


namespace Tricorn
{

using System;
    using System.Collections.Generic;
    
public partial class Dash_MStock
{

    public Nullable<int> Material_Reference { get; set; }

    public double Quantity_In_Stock { get; set; }

    public double Quantity_Expected { get; set; }

    public double Quantity_Received { get; set; }

    public double Quantity_Rejected { get; set; }

    public bool Quarantined { get; set; }

    public Nullable<decimal> Unit_Cost { get; set; }

    public double Cost_Quantity { get; set; }

    public Nullable<decimal> Unit_Sale_Price { get; set; }

    public Nullable<System.DateTime> Received_Date { get; set; }

    public Nullable<System.DateTime> Expiry_Date { get; set; }

    public string Location { get; set; }

    public Nullable<int> PIA_Reference { get; set; }

    public Nullable<int> PItem_Reference { get; set; }

    public string Source { get; set; }

    public string Source_Reference { get; set; }

    public Nullable<int> WOrder_Reference { get; set; }

    public string Batch_Number { get; set; }

    public string Advice_Note { get; set; }

    public string CoC { get; set; }

    public string Supplier_Batch_Number { get; set; }

    public string Stock_Category { get; set; }

    public Nullable<double> Stock_Cost { get; set; }

    public Nullable<double> Standard_Stock_Cost { get; set; }

    public Nullable<double> Stock_Sales_Value { get; set; }

    public Nullable<double> Stock_Sales_Value_Or_Cost { get; set; }

    public string Description { get; set; }

    public string Specification { get; set; }

}

}
