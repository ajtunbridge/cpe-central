    ...
    public sealed class MachinesDataProvider : IDisposable
    {
        ...
        private void InsertSerialMachine(Machine machineToInsert)
        {
            ...
            string cmdText = string.Format(
                @"INSERT INTO machines_serial (name,com_port, stop_bits, parity, handshake, data_bits, baud_rate,
                    dtr_enable, rts_enable, send_xon_char, send_xoff_char, receive_xon_char, receive_xoff_char, program_start,
        ...
        private void UpdateSerialMachine(Machine machineToUpdate)
        {
            ...
            string cmdText = string.Format(
                @"UPDATE machines_serial SET name='{0}', com_port='{1}', stop_bits='{2}', parity='{3}', handshake='{4}', data_bits={5},
                    baud_rate={6}, dtr_enable={7}, rts_enable={8}, send_xon_char={9}, send_xoff_char={10}, receive_xon_char={11},
                    receive_xoff_char={12}, program_start='{13}', program_end='{14}', new_line='{15}', photo=@photo)
                  WHERE id={16}",
                mc.Name,
                mc.ComPort,
                EnumToString(mc.StopBits),
        ...
        private SerialMachine ReaderToSerialMachine(SQLiteDataReader reader)
        {
            var machine = new SerialMachine {
                ...
                SendXOnChar = reader.GetByte(reader.GetOrdinal("send_xon_char")),
                SendXOffChar = reader.GetByte(reader.GetOrdinal("send_xoff_char")),
                ReceiveXOnChar = reader.GetByte(reader.GetOrdinal("receive_xon_char")),
                ReceiveXOffChar = reader.GetByte(reader.GetOrdinal("receive_xoff_char")),
