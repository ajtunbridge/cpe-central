...
namespace EasyProgressBar
{
    ...
    public partial class EasyProgressBar : Control, IProgressBar, IFloatWindowBase
    {
        #region Struct
        ...
        /// </summary>
        [Description("Occurs when the currently docking style of the EasyProgressBar is changed")]
        ...
        /// </summary>
        [Description("Event raised when the EasyProgressBar control is closed by the user, it's docking state should be float window for this change to take effect")]
        public event EventHandler EasyProgressBarClosed;

        public delegate void EasyProgressBarClosingEventHandler(object sender, out bool cancel);
        /// <summary>
        ...
        /// </summary>
        [Description("Occurs when the docking state of the EasyProgressBar control is in the float mode and EasyProgressBar control is being closed by the user")]
        public event EasyProgressBarClosingEventHandler EasyProgressBarClosing;
        ...
        private EasyProgressBarBorderStyle controlBorderStyle = EasyProgressBarBorderStyle.Flat;  // Initializer
        ...
        /// </summary>
        [Description("Provides keyboard support to the user for EasyProgressBar operations[Home(Min value), End(Max value), Left(Decrease current value), Right(Increase current value), Enter(ReDocking), F1 Keys]")]
        [DefaultValue(true)]
        [Browsable(true)]
        ...
        protected virtual void OnEasyProgressBarClosed(EventArgs e)
        {
            if (EasyProgressBarClosed != null)
                EasyProgressBarClosed(this, e);
        }

        protected virtual bool OnEasyProgressBarClosing()
        {
            bool result = false;
            if (EasyProgressBarClosing != null)
                EasyProgressBarClosing(this, out result);
    ...
    public class EasyProgressBarDesigner : ControlDesigner
    {
        ...
        protected EasyProgressBarDesigner()
            : base() { }
        ...
        protected override void PreFilterProperties(System.Collections.IDictionary properties)
        {
            ...
            if (progressBar != null)
            {
                ...
                {
                    properties["BorderColor"] = TypeDescriptor.CreateProperty(typeof(EasyProgressBar),
                ...
                if (!progressBar.IsPaintBorder)
                {
                    properties["BorderColor"] = TypeDescriptor.CreateProperty(typeof(EasyProgressBar),
                      (PropertyDescriptor)properties["BorderColor"], BrowsableAttribute.No);
                    properties["ControlBorderStyle"] = TypeDescriptor.CreateProperty(typeof(EasyProgressBar),
                      (PropertyDescriptor)properties["ControlBorderStyle"], BrowsableAttribute.No);
                }

                if (!progressBar.ShowPercentage)
                {
                    properties["Font"] = TypeDescriptor.CreateProperty(typeof(EasyProgressBar),
                      (PropertyDescriptor)properties["Font"], BrowsableAttribute.No);
                    properties["ForeColor"] = TypeDescriptor.CreateProperty(typeof(EasyProgressBar),
                      (PropertyDescriptor)properties["ForeColor"], BrowsableAttribute.No);
                    properties["DisplayFormat"] = TypeDescriptor.CreateProperty(typeof(EasyProgressBar),
                      (PropertyDescriptor)properties["DisplayFormat"], BrowsableAttribute.No);
                }

                if (!progressBar.IsDigitDrawEnabled)
                {
                    properties["DigitBoxGradient"] = TypeDescriptor.CreateProperty(typeof(EasyProgressBar),
    ...
    public class EasyProgressBarActionList : DesignerActionList
    {
        ...
        private EasyProgressBar linkedControl;
