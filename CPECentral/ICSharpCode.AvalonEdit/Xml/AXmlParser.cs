...
namespace ICSharpCode.AvalonEdit.Xml
{
    ...
    {
        private AXmlDocument userDocument;
        ...
        public AXmlDocument Parse(string input, IEnumerable<DocumentChangeEventArgs> changesSinceLastParse)
        {
            ...
            if (changesSinceLastParse != null) {
                TrackedSegments.UpdateOffsetsAndInvalidate(changesSinceLastParse);
            }
            else {
                TrackedSegments.InvalidateAll();
            }

            var tagReader = new TagReader(this, input);
            ...
            Log("Updating main DOM tree...");
            userDocument.UpdateTreeFrom(parsedDocument);
            userDocument.DebugCheckConsistency(true);
            Assert(userDocument.GetSelfAndAllChildren().Count() == parsedDocument.GetSelfAndAllChildren().Count(),
                "Parsed document and updated document have different number of children");
            return userDocument;
        }
        ...
        {
            UnknownEntityReferenceIsError = true;
            TrackedSegments = new TrackedSegmentCollection();
            userDocument = new AXmlDocument {Parser = this};
            userDocument.Document = userDocument;
            // Track the document
            TrackedSegments.AddParsedObject(userDocument, null);
            userDocument.IsCached = false;
        }
    }
}