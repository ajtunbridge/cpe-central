...
namespace ICSharpCode.AvalonEdit.Xml
{
    ...
    /// </summary>
    internal class TrackedSegmentCollection
    {
        /// <summary>
        ///     Holds all types of objects in one collection.
        /// </summary>
        private readonly TextSegmentCollection<TextSegment> segments = new TextSegmentCollection<TextSegment>();
        ...
        public void UpdateOffsetsAndInvalidate(IEnumerable<DocumentChangeEventArgs> changes)
        {
            foreach (DocumentChangeEventArgs change in changes) {
                ...
                ReadOnlyCollection<TextSegment> segmentsContainingOffset =
                    segments.FindOverlappingSegments(change.Offset, change.InsertionLength);
                ...
                foreach (TouchedRange range in segmentsContainingOffset.OfType<TouchedRange>()) {
                    AXmlParser.Log("Found that {0} dependeds on ({1}-{2})", range.TouchedByObject, range.StartOffset,
                        range.EndOffset);
        ...
        public void AddParsedObject(AXmlObject obj, int? maxTouchedLocation)
        {
            if (!(obj.Length > 0 || obj is AXmlDocument)) {
                AXmlParser.Assert(false,
                    string.Format(CultureInfo.InvariantCulture, "Invalid object {0}.  It has zero length.", obj));
            }
//			// Expensive check
...
//			}
        ...
        public void RemoveParsedObject(AXmlObject obj)
        {
            ...
            var container = obj as AXmlContainer;
            if (container != null) {
        ...
        private IEnumerable<AXmlObject> FindParents(AXmlObject child)
        {
        ...
        private void InvalidateCache(AXmlObject obj, bool includeParents)
        {
            if (includeParents) {
