...
namespace ICSharpCode.AvalonEdit.Xml
{
    internal class TokenReader
    {
        ...
        private int maxTouchedLocation;
        private int stringCacheRequestedCount;
        private int stringCacheRequestedSize;
        private int stringCacheStoredCount;
        private int stringCacheStoredSize;
        ...
        {
            if (currentLocation - back == inputLength) {
                return false;
            }
            if (currentLocation - back < 0) {
                return false;
            }
        ...
        protected bool TryPeek(string text)
        {
            ...
            if (currentLocation + text.Length > inputLength) {
                return false;
            }
        ...
        protected bool TryMoveToNonWhiteSpace(int inputLength)
        {
            while (true) {
                if (currentLocation == inputLength) {
                    return false; // Reject end of file
                }
                char c = input[currentLocation];
                if ((c <= 0x20) && (c == ' ' || c == '\t' || c == '\n' || c == '\r')) {
        ...
        protected bool TryReadName(out string res)
        {
            ...
            while (true) {
                if (currentLocation == inputLength) {
                    break; // Reject end of file
                }
                char c = input[currentLocation];
                if (0x41 <= c) {
                    // Accpet from 'A' onwards
                    currentLocation++;
                    continue;
                }
        ...
        private string GetCachedString(string cached)
        {
        ...
        {
            AXmlParser.Log("String cache: Requested {0} ({1} bytes);  Actaully stored {2} ({3} bytes); {4}% stored",
                stringCacheRequestedCount, stringCacheRequestedSize, stringCacheStoredCount, stringCacheStoredSize,
                stringCacheRequestedSize == 0 ? 0 : stringCacheStoredSize*100/stringCacheRequestedSize);
        }
    }
}