    ...
    public sealed class FoldingSection : TextSegment
    {
                ...
                while (current != endLine.NextLine) {
                    ...
                    if (current == startLine && current == endLine) {
                        ...
                        if (current.EndOffset - StartOffset > 0) {
                            builder.AppendLine(
                                manager.document.GetText(StartOffset, current.EndOffset - StartOffset).TrimStart());
                        }
                    }
                    else if (current == endLine) {
                        if (startIndent.Length <= currentIndent.Length) {
                            builder.Append(manager.document.GetText(current.Offset + startIndent.Length,
                                EndOffset - current.Offset - startIndent.Length));
                        }
                        else {
                            builder.Append(manager.document.GetText(current.Offset + currentIndent.Length,
                                EndOffset - current.Offset - currentIndent.Length));
                        }
                    }
                    else {
                        if (startIndent.Length <= currentIndent.Length) {
                            builder.AppendLine(manager.document.GetText(current.Offset + startIndent.Length,
                                current.Length - startIndent.Length));
                        }
                        else {
                            builder.AppendLine(manager.document.GetText(current.Offset + currentIndent.Length,
        ...
        internal void ValidateCollapsedLineSections()
        {
            ...
            DocumentLine startLine =
                manager.document.GetLineByOffset(StartOffset.CoerceValue(0, manager.document.TextLength));
            DocumentLine endLine =
                manager.document.GetLineByOffset(EndOffset.CoerceValue(0, manager.document.TextLength));
            if (startLine == endLine) {
                ...
                for (int i = 0; i < collapsedSections.Length; i++) {
                    ...
                    if (collapsedSection == null || collapsedSection.Start != startLinePlusOne ||
                        collapsedSection.End != endLine) {
                        // recreate this collapsed section
                        if (collapsedSection != null) {
                            Debug.WriteLine("CollapsedLineSection validation - recreate collapsed section from " +
                                            startLinePlusOne + " to " + endLine);
