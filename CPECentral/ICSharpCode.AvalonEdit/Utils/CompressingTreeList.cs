...
namespace ICSharpCode.AvalonEdit.Utils
{
    ...
    public sealed class CompressingTreeList<T> : IList<T>
    {
            ...
            unchecked {
                if (Count + count < 0) {
                    throw new OverflowException(
                        "Cannot insert elements: total number of elements must not exceed int.MaxValue.");
                }
            }
        ...
        public void RemoveRange(int index, int count)
        {
            ...
            if (count < 0 || index + count > Count) {
                throw new ArgumentOutOfRangeException("count", count,
                    "0 <= length, index(" + index + ")+count <= " + Count);
            }
            if (count == 0) {
                return;
            }
            ...
            if (index + count < n.count) {
                ...
                if (n != null) {
                    ...
                    if (firstNodeBeforeDeletedRange != null &&
                        comparisonFunc(firstNodeBeforeDeletedRange.value, n.value)) {
        ...
        private void CheckNodeProperties(Node node, Node parentNode, bool parentColor, int blackCount,
