...
{
    [Serializable]
    internal sealed class XmlHighlightingDefinition : IHighlightingDefinition2
    {
        private readonly Dictionary<string, HighlightingColor> colorDict = new Dictionary<string, HighlightingColor>();
        [OptionalField] private readonly Dictionary<string, string> propDict = new Dictionary<string, string>();

        private readonly Dictionary<string, HighlightingRuleSet> ruleSetDict =
            new Dictionary<string, HighlightingRuleSet>();
        ...
        private sealed class TranslateElementVisitor : IXshdVisitor
        {
            ...
            private HighlightingRuleSet GetRuleSet(XshdElement position, XshdReference<XshdRuleSet> ruleSetReference)
            {
                ...
                if (ruleSetReference.ReferencedElement != null) {
                    ...
                    if (ruleSet == null) {
                        throw Error(position,
                            "Could not find rule set named '" + ruleSetReference.ReferencedElement + "'.");
                    }
                    return ruleSet;
                }
                return null;
