...
namespace ICSharpCode.AvalonEdit.Rendering
{
    ...
    public sealed class VisualLine
    {
        ...
        /// <summary>
        ///     Gets the Y position of the line. This is measured in device-independent pixels relative to the start of the
        ///     document.
        /// </summary>
        ...
        internal void ConstructVisualElements(ITextRunConstructionContext context,
        ...
        /// <summary>
        ///     Replaces <paramref name="count" /> elements starting at <paramref name="elementIndex" /> with the specified
        ///     elements.
        ///     The replacement operation must preserve the document length, but may change the visual length.
        /// </summary>
        /// <remarks>
        ...
        public void ReplaceElement(int elementIndex, int count, params VisualLineElement[] newElements)
        {
            ...
            if (oldDocumentLength != newDocumentLength) {
                throw new InvalidOperationException("Old elements have document length " + oldDocumentLength +
                                                    ", but new elements have length " + newDocumentLength);
            }
        ...
        public int GetVisualColumn(TextLine textLine, double xPos, bool allowVirtualSpace)
        {
            if (xPos > textLine.WidthIncludingTrailingWhitespace) {
                if (allowVirtualSpace && textLine == TextLines[TextLines.Count - 1]) {
                    var virtualX =
                        (int) Math.Round((xPos - textLine.WidthIncludingTrailingWhitespace)/textView.WideSpaceWidth);
            ...
            bool allowVirtualSpace)
        {
            ...
            if (direction == LogicalDirection.Backward) {
                ...
                if (visualColumn > VisualLength && !elements[elements.Count - 1].HandlesLineBorders &&
                    HasImplicitStopAtLineEnd(mode)) {
                    if (allowVirtualSpace) {
                        return visualColumn - 1;
                    }
                    return VisualLength;
                ...
                if ((allowVirtualSpace || !elements[elements.Count - 1].HandlesLineBorders) &&
                    HasImplicitStopAtLineEnd(mode)) {
        ...
        [SuppressMessage("Microsoft.Usage", "CA1801:ReviewUnusedParameters", MessageId = "mode",
        ...
        #region Nested type: LifetimePhase
