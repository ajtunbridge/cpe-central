    ...
    internal sealed class SelectionMouseHandler : ITextAreaInputHandler
    {
                ...
                if (effect != DragDropEffects.None) {
                    ...
                    if (text != null) {
                        ...
                        if (mode == SelectionMode.Drag && textArea.Selection.Contains(start)) {
                            ...
                            try {
                                if (rectangular &&
                                    RectangleSelection.PerformRectangularPaste(textArea, textArea.Caret.Position, text,
                                        true)) {
                                }
                                else {
        ...
        private void StartDrag()
        {
            ...
            List<AnchorSegment> deleteOnMove =
                textArea.Selection.Segments.Select(s => new AnchorSegment(textArea.Document, s)).ToList();
        ...
        private void textArea_QueryCursor(object sender, QueryCursorEventArgs e)
        {
            if (!e.Handled) {
                if (mode != SelectionMode.None || !enableTextDragDrop) {
                    ...
                    if (p.X >= 0 && p.Y >= 0 && p.X <= textArea.TextView.ActualWidth &&
                        p.Y <= textArea.TextView.ActualHeight) {
                        int visualColumn;
                    ...
                    if ((modifiers & ModifierKeys.Alt) == ModifierKeys.Alt &&
                        ...
                        if (shift && !textArea.Selection.IsEmpty) {
                            if (startWord.Offset < textArea.Selection.SurroundingSegment.Offset) {
                                textArea.Selection =
                                    textArea.Selection.SetEndpoint(
                                        new TextViewPosition(textArea.Document.GetLocation(startWord.Offset)));
                            }
                            else if (startWord.EndOffset > textArea.Selection.SurroundingSegment.EndOffset) {
                                textArea.Selection =
                                    textArea.Selection.SetEndpoint(
                                        new TextViewPosition(textArea.Document.GetLocation(startWord.EndOffset)));
                            }
        ...
        private void textArea_MouseMove(object sender, MouseEventArgs e)
        {
            ...
            if (mode == SelectionMode.Normal || mode == SelectionMode.WholeWord || mode == SelectionMode.WholeLine ||
                mode == SelectionMode.Rectangular) {
                e.Handled = true;
        ...
        private void ExtendSelectionToMouse(MouseEventArgs e)
        {
            ...
            if (mode == SelectionMode.Normal || mode == SelectionMode.Rectangular) {
                ...
                SimpleSegment newWord = (mode == SelectionMode.WholeLine)
                    ? GetLineAtMousePosition(e)
        ...
        private void textArea_MouseLeftButtonUp(object sender, MouseButtonEventArgs e)
        {
            ...
            else if (mode == SelectionMode.Normal || mode == SelectionMode.WholeWord || mode == SelectionMode.WholeLine ||
                     mode == SelectionMode.Rectangular) {
                ExtendSelectionToMouse(e);
            }
            mode = SelectionMode.None;
