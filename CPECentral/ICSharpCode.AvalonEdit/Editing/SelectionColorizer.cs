        ...
        protected override void Colorize(ITextRunConstructionContext context)
        {
            ...
            foreach (SelectionSegment segment in textArea.Selection.Segments) {
                ...
                if (segmentStart < lineStartOffset) {
                    ...
                    startColumn = context.VisualLine.ValidateVisualColumn(segment.StartOffset, segment.StartVisualColumn,
                        textArea.Selection.EnableVirtualSpace);
                }

                int endColumn;
                if (segmentEnd > lineEndOffset) {
                    endColumn = textArea.Selection.EnableVirtualSpace
                        ? int.MaxValue
                        : context.VisualLine.VisualLengthWithEndOfLineMarker;
                }
                else {
                    endColumn = context.VisualLine.ValidateVisualColumn(segment.EndOffset, segment.EndVisualColumn,
                        textArea.Selection.EnableVirtualSpace);
                }
